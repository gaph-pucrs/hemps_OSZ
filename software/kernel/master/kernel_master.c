...
void send_io_config(Application* app, int appID_rand, int turns){
	...
	puts("AppID rand: ");puts(itoh(appID_rand));puts("\n");
	puts("turns: ");puts(itoh(turns));puts("\n");
	puts("----Sending configs:");puts(itoa(MemoryRead(TICK_COUNTER)));puts("\n");
	...
	for (int i =0; i<app->tasks_number; i++){
		...
		for(int j =0; j < app->tasks[i].dependences_number; j++){
			for (int k = 0; k < IO_NUMBER; k++){
				...
				if(usedIO[k] == 0){
					puts("----Enviando conf para: ");puts(itoa(app->tasks[i].dependences[j].flits));puts("\n");
					puts("START NEW IO PATH:");puts(itoa(MemoryRead(TICK_COUNTER)));puts("\n");
					...
					puts("END NEW IO PATH:  ");puts(itoa(MemoryRead(TICK_COUNTER)));puts(" path size:");puts(itoa(sr.path_size));puts("\n");
					break;
				}
	...
	puts("----IO confs enviados");puts(itoa(MemoryRead(TICK_COUNTER)));puts("\n");
}
		...
		for(int j=0; j<app->tasks[i].dependences_number; j++) {
			for(int k = 0; k<IO_NUMBER; k++) {
				...
				if(usedIO[k] == 0) {
					...
					puts("----Enviando renew para: ");puts(itoa(app->tasks[i].dependences[j].flits));puts("\n");
					usedIO[k] = app->tasks[i].dependences[j].flits;
					k0 = get_NI_k0(usedIO[k]);
...
void changeAPlocation(Application *app){
	...
	LL_x = app->LL_Address >> 4;
	LL_y = app->LL_Address & 0xf;
	RH_x = app->RH_Address >> 4;
	RH_y = app->RH_Address & 0xf;
	AP_x = app->ap.address_go >> 8;
	AP_y = app->ap.address_go & 0xf;

	puts("Changing AP from: "); puts(itoh(app->ap.address_go));puts("   start:"); puts(itoa(MemoryRead(TICK_COUNTER)));puts("\n");
	puts("LL / RH "); puts(itoh(app->LL_Address));puts(" // "); puts(itoh(app->RH_Address));puts("\n");

	if (app->ap.port_go == NORTH){ // AP_X == LL_X
		if (AP_x == LL_x){
			AP_x = RH_x;
		}else{
			AP_x--;
		...
		app->ap.address_go   = ( AP_x << 8 ) | AP_y;
      	...
      	app->ap.port_go = NORTH;
      	app->ap.port_back = NORTH;
	}
