...
int IOtoAP(int peripheral_id){ // Colocar parametro com o AP addr
  ...
  for(i = 0; i < IO_NUMBER; i++){
      if(io_info[i].peripheral_id == peripheral_id){
          my_X_addr = io_info[i].default_address_x ;
          my_Y_addr = io_info[i].default_address_y;
          port_io = io_info[i].default_port;
          break;
      }
  }
  ...
  // CASO AP NO NORTE -- Lados até o X do AP, depois SUL pra entrar na SZ
  if (medium_X != -1){ // AP at North
	...
	// Manda pra BAIXO pra "entrar"
	bt = bt | (0x3 << shift);
    shift += 2;
    auxPosY ++;
	bt = bt | (0x2 << shift); //Finalizar o BT
	return bt;
  }

  // CASO DEFAULT -- Lados até o X da GA tangente ao AP, depois SUL até o Y do AP, mais uma E ou W pra entrar na SZ
  else if(ga.cols[MAX_GRAY_COLS-1] < LL_X_addr){    
    // puts("--Gray Area a esquerda\n");
    while (auxPosX >= LL_X_addr){
      bt = bt | (0x1 << shift);
