...
//putsv("\nSC: ",current_time);
	...
	if (scheduled_task == 0){
		...
		for(int i=0; i<MAX_LOCAL_TASKS; i++){
			...
			switch(task->status){
				case FREE		: sched_puts("FREE\n"); break;
				case READY		: sched_puts("READY\n"); break;
				case WAITING	: sched_puts("WAITING\n"); break;
				case RUNNING	: sched_puts("RUNNING\n"); break;
				case MIGRATING	: sched_puts("MIGRATING\n"); break;
				case BLOCKED	: sched_puts("BLOCKED\n"); break;
				case SLEEPING	: sched_puts("SLEEPING\n"); break;
			}
