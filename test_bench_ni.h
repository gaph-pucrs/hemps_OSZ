...
#include "./ni/NetworkInterfaceWrapped2.h"
...
SC_MODULE(test_bench) {
    ...
    hemps *MPSoC;
    injector *Injector;
    NetworkInterfaceWrapped * ptr_ni;
    ...
    sc_module(name_), filename(filename_)
    {
        ...
        ptr_ni = new NetworkInterfaceWrapped("NI");
        
        ptr_ni->clock(clock);
        ptr_ni->reset(reset);

        ptr_ni->hermes_primary_tx(rx_tb[2]);
        ptr_ni->hermes_primary_tx_clk(clock_rx_tb[2]);
        ptr_ni->hermes_primary_data_out(data_in_tb[2]);
        ptr_ni->hermes_primary_eop_out(eop_in_tb[2]);
        ptr_ni->hermes_primary_credit_in(credit_o_tb[2]);

        ptr_ni->hermes_primary_rx(tx_tb[2]);
        ptr_ni->hermes_primary_rx_clk(clock_tx_tb[2]);
        ptr_ni->hermes_primary_data_in(data_out_tb[2]);
        ptr_ni->hermes_primary_eop_in(eop_out_tb[2]);  
        ptr_ni->hermes_primary_credit_out(credit_i_tb[2]);
  
        ptr_ni->brnoc_primary_source_in(out_source_router_seek_tb[2]);
        ptr_ni->brnoc_primary_target_in(out_target_router_seek_tb[2]);
        ptr_ni->brnoc_primary_service_in(out_service_router_seek_tb[2]);
        ptr_ni->brnoc_primary_payload_in(out_payload_router_seek_tb[2]);
        ptr_ni->brnoc_primary_req_in(out_req_router_seek_tb[2]);
        ptr_ni->brnoc_primary_opmode_in(out_opmode_router_seek_tb[2]);
        ptr_ni->brnoc_primary_ack_out(in_ack_router_seek_tb[2]);

        ptr_ni->brnoc_primary_source_out(in_source_router_seek_tb[2]);
        ptr_ni->brnoc_primary_target_out(in_target_router_seek_tb[2]);
        ptr_ni->brnoc_primary_service_out(in_service_router_seek_tb[2]);
        ptr_ni->brnoc_primary_payload_out(in_payload_router_seek_tb[2]);
        ptr_ni->brnoc_primary_req_out(in_req_router_seek_tb[2]);
        ptr_ni->brnoc_primary_opmode_out(in_opmode_router_seek_tb[2]);
        ptr_ni->brnoc_primary_ack_in(out_ack_router_seek_tb[2]);
