...
#include "./peripherals/IO_peripheral/io_peripheral.h"
...
SC_MODULE(test_bench) {
    ...
    hemps *MPSoC;
    injector *Injector;
    io_peripheral * ptr_io_peripheral;
    ...
    sc_module(name_), filename(filename_)
    {
        ...
        ptr_io_peripheral = new io_peripheral("IO_PERIPHERAL", 80);
        ptr_io_peripheral->clock(clock);
        ptr_io_peripheral->reset(reset);
 ...
 // NoC Interface
        ...
        ptr_io_peripheral->clock_tx_primary(clock_rx_tb[2]);
        ptr_io_peripheral->tx_primary(rx_tb[2]);
        ptr_io_peripheral->data_out_primary(data_in_tb[2]);
        ptr_io_peripheral->credit_i_primary(credit_o_tb[2]);
        ptr_io_peripheral->eop_in_primary(eop_out_tb[2]);  
        ptr_io_peripheral->clock_rx_primary(clock_tx_tb[2]);
        ptr_io_peripheral->rx_primary(tx_tb[2]);
        ptr_io_peripheral->data_in_primary(data_out_tb[2]);
        ptr_io_peripheral->credit_o_primary(credit_i_tb[2]);
        ptr_io_peripheral->eop_out_primary(eop_in_tb[2]);

  
        ptr_io_peripheral->in_source_router_seek_primary(out_source_router_seek_tb[2]);
        ptr_io_peripheral->in_target_router_seek_primary(out_target_router_seek_tb[2]);
        ptr_io_peripheral->in_payload_router_seek_primary(out_payload_router_seek_tb[2]);
        ptr_io_peripheral->in_service_router_seek_primary(out_service_router_seek_tb[2]);
        ptr_io_peripheral->in_req_router_seek_primary(out_req_router_seek_tb[2]);
        ptr_io_peripheral->in_ack_router_seek_primary(out_ack_router_seek_tb[2]);
        ptr_io_peripheral->in_opmode_router_seek_primary(out_opmode_router_seek_tb[2]);

        ptr_io_peripheral->out_service_router_seek_primary(in_service_router_seek_tb[2]);
        ptr_io_peripheral->out_source_router_seek_primary(in_source_router_seek_tb[2]);
        ptr_io_peripheral->out_target_router_seek_primary(in_target_router_seek_tb[2]);
        ptr_io_peripheral->out_payload_router_seek_primary(in_payload_router_seek_tb[2]);
        ptr_io_peripheral->out_ack_router_seek_primary(in_ack_router_seek_tb[2]);
        ptr_io_peripheral->out_nack_router_seek_primary(in_nack_router_seek_tb[2]);
        ptr_io_peripheral->out_opmode_router_seek_primary(in_opmode_router_seek_tb[2]);
        ptr_io_peripheral->out_req_router_seek_primary(in_req_router_seek_tb[2]);
