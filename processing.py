...
def process_directory(size, delay, payload):
    # Input directory
    input_dir = f"sandbox/intermittent_campaign_large/size{size}_delay{delay}/"
...
if __name__ == "__main__":
    ...
    sizes = [5, 10, 30]       # Example sizes
    delays = [10, 50, 100, 250]     # Example delays

    # Process all combinations
    for size in sizes:
        for delay in delays:
            print(f"\nProcessing size={size}, delay={delay}, payload={30}")
            process_directory(size, delay, 30)
