	...
	//	if((RouterPosition(i) == BR || RouterPosition(i) == CRX || RouterPosition(i) == TR)){
		...
		//if((RouterPosition(i) == TL || RouterPosition(i) == TC || RouterPosition(i) == TR)){
 		...
 		if(open_io[i] == 3 && (RouterPosition(i) == BL || RouterPosition(i) == BC || RouterPosition(i) == BR)){
				...
				in_source_router_seek	[i][SOUTH].write(sig_out_source_router_seek_io[io_count].read());
				in_target_router_seek	[i][SOUTH].write(sig_out_target_router_seek_io[io_count].read());
				in_payload_router_seek	[i][SOUTH].write(sig_out_payload_router_seek_io[io_count].read());
				in_service_router_seek	[i][SOUTH].write(sig_out_service_router_seek_io[io_count].read());
				in_req_router_seek		[i][SOUTH].write(sig_out_req_router_seek_io[io_count].read());
				//in_ack_router_seek		[i][SOUTH].write(1);
				in_ack_router_seek		[i][SOUTH].write(sig_out_ack_router_seek_io[io_count].read());
				in_nack_router_seek		[i][SOUTH].write(sig_out_nack_router_seek_io[io_count].read());
				in_opmode_router_seek	[i][SOUTH].write(sig_out_opmode_router_seek_io[io_count].read());
