...
int main()
{
	...
	// IO: get plain text message
	for(x = 0; x < MSG_LENGHT; x += MSG_SIZE)
	{
		msgIO.length = (MSG_LENGHT - x > MSG_SIZE) ? MSG_SIZE : MSG_LENGHT - x;
		IOReceive(&msgIO, IO_PERIPHERAL);
		//Echo(".");
	}
	...
	//	Calculate number of blocks/messages to send
	//   to each Slave_PE
